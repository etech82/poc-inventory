...
import com.walgreens.rxi.inventory.config.KafkaProperties;
...
class InventoryKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/inventory-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/inventory-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
